      ...
      <search className="w-full max-w-[648px] min-h-[302px] flex flex-col items-center justify-center">
        ...
        <ul className="w-full flex flex-wrap justify-center items-center list-none gap-4 py-6">
          {mockData.map((item) => (
            <li
              key={item.id}
              className={`px-6 py-2 text-center bg-[#FFDAD2]/40 border border-[#FF7A2C]/10 rounded-full ${item.isError ? "flex items-center gap-2" : ""}`}
            >
              <span>{item.name}</span>
              {item.isError && (
                <Image
        ...
        <Link
          href="/recipes"
          className="w-full min-h-[96px] flex items-center justify-center"
        >
          <Button
            variant="gradient"
